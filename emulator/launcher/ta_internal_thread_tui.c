...
static bool init_request(struct array *msg, struct ta_task **new_ta_task, uint8_t msg_name)
{
...
static bool wait_for_response(struct com_msg_tui_ta2display **msg, struct array *msg_array)
{
...
static TEE_Result wait_and_handle_tui_check_text_format(uint32_t *width, uint32_t *height,
							uint32_t *lastIndex)
{
	struct com_msg_tui_ta2display *msg = NULL;
	struct array msg_array = {0, sizeof(struct com_msg_tui_ta2display), NULL};
	...
	if (!cmp_read_array(&cmp, &array_size) || array_size != 4 || !cmp_read_uint(&cmp, &ret) ||
	    !cmp_read_uint(&cmp, width) || !cmp_read_uint(&cmp, height) ||
...
TEE_Result ta_tui_check_text_format(char *text, uint32_t *width, uint32_t *height,
				    uint32_t *lastIndex)
{
	...
	if (!cmp_write_array(&cmp, 1) || !cmp_write_str(&cmp, text, strlen(text))) {
...
static bool deserialize_buttoninfo(cmp_ctx_t *cmp, TEE_TUIScreenButtonInfo *buttonInfo, uint32_t i)
{
	uint32_t array_size = 0;
	...
	if (!cmp_read_array(cmp, &array_size) || array_size != 5 ||
...
static bool deserialize_buttoninfo_array(cmp_ctx_t *cmp, TEE_TUIScreenInfo *screenInfo)
{
	uint32_t array_size = 0;
	uint32_t i;

	if (!cmp_read_array(cmp, &array_size) || array_size != TEE_TUI_NUMBER_BUTTON_TYPES)
		return false;
...
static TEE_Result wait_and_handle_tui_get_screen_info(TEE_TUIScreenInfo *screenInfo)
{
	...
	/* Deserialization */
	if (!cmp_read_array(&cmp, &array_size) || array_size != 17 || !cmp_read_uint(&cmp, &ret) ||
				  ...
				  uint32_t nbEntryFields, TEE_TUIScreenInfo *screenInfo)
{
	...
	/* Serialization */
	if (!cmp_write_array(&cmp, 2) || !cmp_write_uint(&cmp, screenOrientation) ||
...
static TEE_Result wait_and_handle_tui_init_session()
{
	...
	/* Deserialization */
	if (!cmp_read_array(&cmp, &array_size) || array_size != 1 || !cmp_read_uint(&cmp, &ret))
		OT_LOG(LOG_ERR, "Deserialization error: %s", cmp_strerror(&cmp));
...
static bool deserialize_entryfields(cmp_ctx_t *cmp, TEE_TUIEntryField *entryFields,
				    uint32_t entryFieldCount)
{
	...
	if (!cmp_read_array(cmp, &array_size) || array_size != entryFieldCount) {
						     ...
						     TEE_TUIButtonType *selectedButton)
{
	...
	/* Deserialization */
	if (!cmp_read_array(&cmp, &array_size) || array_size != 3 || !cmp_read_uint(&cmp, &ret) ||
...
static bool serialize_image(cmp_ctx_t *cmp, TEE_TUIImage *image)
{
	if (!cmp_write_array(cmp, 4) || !cmp_write_uint(cmp, image->source))
		return false;
	...
	if (!cmp_write_uint(cmp, image->width) || !cmp_write_uint(cmp, image->height))
		return false;

	return true;
}

static bool serialize_screenlabel(cmp_ctx_t *cmp, TEE_TUIScreenLabel *label)
{
	/* text can be null */
	char *text = label->text == NULL ? "\0" : label->text;

	if (!cmp_write_array(cmp, 9) || !cmp_write_str(cmp, text, strlen(text)) ||
	    !cmp_write_uint(cmp, label->textXOffset) || !cmp_write_uint(cmp, label->textYOffset) ||
	    !cmp_write_u8(cmp, label->textColor[0]) || !cmp_write_u8(cmp, label->textColor[1]) ||
	    !cmp_write_u8(cmp, label->textColor[2]) || !serialize_image(cmp, &label->image) ||
	    !cmp_write_uint(cmp, label->imageXOffset) || !cmp_write_uint(cmp, label->imageYOffset))
		return false;

	return true;
}

static bool serialize_button(cmp_ctx_t *cmp, TEE_TUIButton *button)
{
	bool defaults;
	char *text;
	TEE_TUIImage *image;
	...
	if (!cmp_write_array(cmp, 3) || !cmp_write_bool(cmp, defaults))
		return false;
	...
	if (!cmp_write_str(cmp, text, strlen(text)) || !serialize_image(cmp, image))
		return false;

	return true;
}

static bool serialize_buttons(cmp_ctx_t *cmp, TEE_TUIScreenConfiguration *screenConfiguration)
{
	uint32_t i;
...
static bool serialize_entryfield(cmp_ctx_t *cmp, TEE_TUIEntryField *entryField)
{
	if (!cmp_write_array(cmp, 6) ||
	    !cmp_write_str(cmp, entryField->label, strlen(entryField->label)) ||
	    !cmp_write_uint(cmp, entryField->mode) || !cmp_write_uint(cmp, entryField->type) ||
...
static bool serialize_entryfields(cmp_ctx_t *cmp, TEE_TUIEntryField *entryFields,
				  uint32_t entryFieldCount)
{
	uint32_t i;
...
TEE_Result ta_tui_display_screen(TEE_TUIScreenConfiguration *screenConfiguration,
				 ...
				 uint32_t entryFieldCount, TEE_TUIButtonType *selectedButton)
{
	struct ta_task *new_ta_task = NULL;
	cmp_ctx_t cmp;
	struct array msg = {0, 0, NULL};
	...
	return wait_and_handle_tui_display_screen(closeTUISession, entryFields, entryFieldCount,
						  selectedButton);
