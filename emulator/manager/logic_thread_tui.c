...
static bool tui_validate_display_init_msg(void *msg, size_t msg_len)
{
	...
	if (msg_len != sizeof(struct com_msg_tui_display_init) +
			   init_msg->buttonInfo[0].textLength + init_msg->buttonInfo[1].textLength +
			   init_msg->buttonInfo[2].textLength + init_msg->buttonInfo[3].textLength +
			   init_msg->buttonInfo[4].textLength + init_msg->buttonInfo[5].textLength +
...
static bool tui_state_requests_add(struct trusted_ui_state *dst, uint64_t id, proc_t request)
{
	if (dst == NULL) {
		return false;
	}
	...
	for (size_t i = 0; i < dst->requests_count; ++i) {
		if (dst->requests[i].id == id) {
			if (i < dst->requests_count - 1) {
				memmove(&(dst->requests[i]), &(dst->requests[i + 1]),
...
void tui_check_text_format(struct manager_msg *man_msg)
{
	...
	if (tui_state.state != TUI_INITIALIZED && tui_state.state != TUI_SESSION &&
...
void tui_get_screen_info(struct manager_msg *man_msg)
{
	...
	memcpy(response_man_msg->msg, tui_state.screen_info_data, tui_state.screen_info_data_size);
...
void tui_close_session(struct manager_msg *man_msg)
{
	...
	if (tui_state.state == TUI_DISPLAY && tui_state.TA_session_lock == man_msg->proc) {
	...
	if (tui_state.state != TUI_SESSION || tui_state.TA_session_lock != man_msg->proc) {
