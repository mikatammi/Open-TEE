...
static TEE_Result not_implemented_integer(uint64_t *i)
{
	(void)i;
	return TEE_ERROR_NOT_IMPLEMENTED;
}

static TEE_Result get_ta_config(struct gpd_ta_config *ta_config)
{
	TEE_Result ret;
	struct com_mgr_invoke_cmd_payload returnPayload = {0};
	if (ta_config == NULL)
		return TEE_ERROR_BAD_PARAMETERS;
	ret = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_GET_TA_CONFIG, NULL,
				   &returnPayload);
	if (ret != TEE_SUCCESS)
		goto err;
	if (returnPayload.size != sizeof(struct gpd_ta_config)) {
		OT_LOG(LOG_ERR,
		       "returnPayload.size = %zu does not match sizeof(struct gpd_ta_config) = %zu",
		       returnPayload.size, sizeof(struct gpd_ta_config));
		ret = TEE_ERROR_BAD_STATE;
	}
	*ta_config = *((struct gpd_ta_config *)returnPayload.data);
err:
	if (returnPayload.data != NULL)
		free(returnPayload.data);
	return ret;
}

static TEE_Result gpd_ta_appID(TEE_UUID *uuid)
{
	struct gpd_ta_config ta_config;
	TEE_Result ret = get_ta_config(&ta_config);
	if (ret != TEE_SUCCESS)
		return ret;
	*uuid = ta_config.appID;
	return TEE_SUCCESS;
}

static TEE_Result gpd_ta_singleInstance(bool *b)
{
	struct gpd_ta_config ta_config;
	TEE_Result ret = get_ta_config(&ta_config);
	if (ret != TEE_SUCCESS)
		return ret;
	*b = ta_config.singletonInstance;
	return TEE_SUCCESS;
}

static TEE_Result gpd_ta_multiSession(bool *b)
{
	struct gpd_ta_config ta_config;
	TEE_Result ret = get_ta_config(&ta_config);
	if (ret != TEE_SUCCESS)
		return ret;
	*b = ta_config.multiSession;
	return TEE_SUCCESS;
}

static TEE_Result gpd_ta_instanceKeepAlive(bool *b)
{
	struct gpd_ta_config ta_config;
	TEE_Result ret = get_ta_config(&ta_config);
	if (ret != TEE_SUCCESS)
		return ret;
	*b = ta_config.instanceKeepAlive;
	return TEE_SUCCESS;
}

static TEE_Result gpd_ta_dataSize(uint64_t *i)
{
	struct gpd_ta_config ta_config;
	TEE_Result ret = get_ta_config(&ta_config);
	if (ret != TEE_SUCCESS)
		return ret;
	*i = ta_config.dataSize;
	return TEE_SUCCESS;
}

static TEE_Result gpd_ta_stackSize(uint64_t *i)
{
	struct gpd_ta_config ta_config;
	TEE_Result ret = get_ta_config(&ta_config);
	if (ret != TEE_SUCCESS)
		return ret;
	*i = ta_config.stackSize;
	return TEE_SUCCESS;
}

static TEE_Result gpd_ta_version(char *buf, size_t *len)
{
	/* Setting this to version 0.1 for now */
	const char version[] = "0.1";
	return update_len_then_strncpy(buf, len, version, sizeof(version));
}

static TEE_Result gpd_ta_description(char *buf, size_t *len)
{
	/* TODO: Somehow maybe get soname here or something, maybe even using
	 * the same "propertie" API the above get_ta_config uses. */
	const char desc[] = "Trusted Application";
	return update_len_then_strncpy(buf, len, desc, sizeof(desc));
}
...
{
	/* TODO: Change to false when those TODOs from opentee_manager_storage_api.c around places
	 * returning TEE_ERROR_CORRUPT_OBJECT have been done. There are TODOs for deleting and
	 * closing object if it is corrupt. Currently it does neither delete or close the object. */
	*b = true;
	return TEE_SUCCESS;
}
...
{
	/* TODO: Implement properly, as libtee/src/tee_client_api.c requires this to be LOGIN_PUBLIC
	 * for now, this implementation is enough for client side connections. However TAs can also
	 * connect to TAs. */
...
struct OT_TEE_Property TA_Properties[] = {
    // TODO: Change to {.uuid = ...}, etc. ...
    {"gpd.ta.appID", OT_TEE_PROPTYPE_UUID, .f.uuid = &gpd_ta_appID},
    {"gpd.ta.singleInstance", OT_TEE_PROPTYPE_BOOLEAN, .f.boolean = &gpd_ta_singleInstance},
    {"gpd.ta.multiSession", OT_TEE_PROPTYPE_BOOLEAN, .f.boolean = &gpd_ta_multiSession},
    {"gpd.ta.instanceKeepAlive", OT_TEE_PROPTYPE_BOOLEAN, .f.boolean = &gpd_ta_instanceKeepAlive},
    {"gpd.ta.dataSize", OT_TEE_PROPTYPE_INTEGER, .f.integer = &gpd_ta_dataSize},
    {"gpd.ta.stackSize", OT_TEE_PROPTYPE_INTEGER, .f.integer = &gpd_ta_stackSize},
    {"gpd.ta.version", OT_TEE_PROPTYPE_STRING, .f.string = &gpd_ta_version},
    {"gpd.ta.description", OT_TEE_PROPTYPE_STRING, .f.string = &gpd_ta_description},
...
static TEE_Result OT_UUIDAsString(char *buf, size_t *len, TEE_UUID u)
{
	...
	*len = uuid_string_len;
	...
	sret = snprintf(buf, orig_len, fmt, u.timeLow, u.timeMid, u.timeHiAndVersion,
				   ...
				   char *valueBuffer, size_t *valueBufferLen)
{
	...
	if (p->type == OT_TEE_PROPTYPE_STRING)
		...
		int mbedret = mbedtls_base64_encode((unsigned char *)valueBuffer, *valueBufferLen,
						    valueBufferLen, (const unsigned char *)tmp_buf,
						    tmp_buf_len);
		if (mbedret == MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL)
			ret = TEE_ERROR_SHORT_BUFFER;
		else
			ret = mbedret ? TEE_ERROR_GENERIC : TEE_SUCCESS;
	err_binary:
		free(tmp_buf);
		return ret;
		...
		int sret = snprintf(tmp_buf, sizeof(tmp_buf), "%lu", val);
		if (sret < 0)
			TEE_Panic(TEE_ERROR_GENERIC);
		...
		*valueBufferLen = strlen(val_as_string) + 1;
		...
		int sret = snprintf(tmp_buf, tmp_buf_len, fmt, identity.login, uuid_str);
		if (sret < 0)
			TEE_Panic(TEE_ERROR_GENERIC);
				 ...
				 TEE_UUID *value)
{
	...
	if (name == NULL && (propsetOrEnumerator == TEE_PROPSET_CURRENT_TA ||
			     propsetOrEnumerator == TEE_PROPSET_CURRENT_CLIENT ||
			     propsetOrEnumerator == TEE_PROPSET_TEE_IMPLEMENTATION))
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
