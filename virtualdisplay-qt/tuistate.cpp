...
TUIState::TUIState(QObject *parent)
    : QObject(parent), current_state_(TUI_STATE_DISCONNECTED), settings_()
{
}
...
{
	uint32_t ret =
	    req.nbEntryFields > settings_.maxEntryFields ? TEE_ERROR_NOT_SUPPORTED : TEE_SUCCESS;

	std::array<TUIProtocol::ButtonInfo, 6> buttonInfos;
	std::transform(settings_.buttonInfos.begin(), settings_.buttonInfos.end(),
		       buttonInfos.begin(), [](const ButtonInfo &in) -> TUIProtocol::ButtonInfo {
			       return TUIProtocol::ButtonInfo(
				   in.buttonText.toStdString(), in.buttonWidth, in.buttonHeight,
				   in.buttonTextCustom, in.buttonImageCustom);
		       });

	return TUIProtocol::GetScreenInfoResponse(
	    ret, settings_.grayscaleBitsDepth, settings_.redBitsDepth, settings_.greenBitsDepth,
	    settings_.blueBitsDepth, settings_.widthInch, settings_.heightInch,
	    settings_.maxEntryFields, settings_.entryFieldLabelWidth,
	    settings_.entryFieldLabelHeight, settings_.maxEntryFieldLength, settings_.labelColor[0],
	    settings_.labelColor[1], settings_.labelColor[2], settings_.labelWidth,
	    settings_.labelHeight, buttonInfos);
}
...
void TUIState::connected()
{
	...
	current_state_ =
	    current_state_ == TUI_STATE_DISCONNECTED ? TUI_STATE_CONNECTED : current_state_;
}
