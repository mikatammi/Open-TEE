...
TrustedUIWidget::TrustedUIWidget(QWidget *parent)
    : QWidget(parent), state_(new TUIState), socket_(), service_(state_)
{
	// Connect Socket to Service
	connect(&socket_, SIGNAL(messageReceived(const ComProtocolMessage &)), &service_,
		SLOT(messageReceived(const ComProtocolMessage &)));
	connect(&service_, SIGNAL(sendMessage(const ComProtocolMessage &)), &socket_,
		SLOT(sendMessage(const ComProtocolMessage &)));

	// TODO: HACK
	connect(&service_, SIGNAL(displayScreen(TUIProtocol::DisplayScreenRequest)), this,
...
void TrustedUIWidget::changeColor()
{
	...
	pal.setColor(QPalette::Base, QColor(rand() % 255, rand() % 255, rand() % 255));
...
void TrustedUIWidget::stop() { socket_.stop(); }
...
void TrustedUIWidget::sendDisplayInitMsg()
{
	...
	com_msg_tui_display_init init_msg{
	    com_msg_hdr{0, COM_MSG_NAME_TUI_DISPLAY_INIT, COM_TYPE_QUERY, {0, 0, 0, 0}, 0},
	    5000, // timeout
	...
	QByteArray raw_msg(reinterpret_cast<char *>(&init_msg), static_cast<int>(sizeof(init_msg)));
...
void TrustedUIWidget::displayScreen(TUIProtocol::DisplayScreenRequest req)
{
	...
	connect(btn, SIGNAL(clicked()), this, SLOT(respond()));
...
void TrustedUIWidget::respond()
{
	...
	ComProtocolMessage respo(COM_MSG_NAME_TUI_DISPLAY_SCREEN, COM_TYPE_RESPONSE, 0, response);
