...
class TUIService : public QObject
{
	Q_OBJECT
      ...
      public:
	explicit TUIService(QSharedPointer<TUIState> state, QObject *parent = NULL);
	~TUIService();
...
{
      public:
	ByteArrayStream(QByteArray *ba) : bytearray_(ba) {}
...
template <typename MSGTYPE> void msgpack_unpack(const QByteArray &msg, MSGTYPE &msgpack_struct)
{
	qDebug() << msg;
...
template <typename MSGTYPE> void msgpack_pack(QByteArray &msg, MSGTYPE &msgpack_struct)
{
