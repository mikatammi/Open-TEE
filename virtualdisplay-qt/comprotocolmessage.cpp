...
ComProtocolMessage::ComProtocolMessage(const com_msg_hdr header, const QByteArray &payload)
{
...
ComProtocolMessage::ComProtocolMessage(const uint8_t msg_name, const uint8_t msg_type,
				       const uint64_t sess_id, const QByteArray &payload)
    : ComProtocolMessage(com_msg_hdr{sess_id, msg_name, msg_type, {0, 0, 0, 0}, 0}, payload)
{
}

ComProtocolMessage::ComProtocolMessage(const QByteArray &raw_message) : data_(raw_message) {}
...
QByteArray ComProtocolMessage::getRawData() const { return data_; }
