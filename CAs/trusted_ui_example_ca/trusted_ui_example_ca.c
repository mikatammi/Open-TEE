...
static int decrypt_aes_key(mbedtls_rsa_context *rsa_ctx, mbedtls_ctr_drbg_context *ctr_drbg_ctx,
			   const char *encrypted_key_buf, char *aes_key, char *aes_iv)
{
	int ret = -1;
	size_t output_len = 0;
	...
	ret = mbedtls_rsa_pkcs1_decrypt(rsa_ctx, mbedtls_ctr_drbg_random, ctr_drbg_ctx, &output_len,
					(const unsigned char *)encrypted_key_buf, decrypted_buf,
					modulus_len);
	if (ret != 0)
		goto err;
...
static int invoke_ta_tui_cmd(struct ta_output *output, struct rsa_key_modulus *pubkey,
			     TEEC_Context *context, TEEC_Session *session)
{
	...
	op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_WHOLE, TEEC_MEMREF_WHOLE, TEEC_MEMREF_WHOLE,
					 TEEC_VALUE_OUTPUT);
	...
	if (TEEC_RegisterSharedMemory(context, &shmem_rsa_encrypted_aes_key_out) != TEEC_SUCCESS)
		goto err2;

	if (TEEC_RegisterSharedMemory(context, &shmem_aes_encrypted_user_data) != TEEC_SUCCESS)
		goto err3;
...
static int invoke_ta(struct ta_output *output, struct rsa_key_modulus *pubkey)
{
...
static int deserialize_data(struct trusted_input *dest, char *buf, size_t buf_len)
{
	...
	if (dest->username == NULL || dest->password == NULL || dest->pincode == NULL) {
		free(dest->username);
		free(dest->password);
		free(dest->pincode);
...
static int extract_modulus_from_rsa(struct rsa_key_modulus *n, mbedtls_rsa_context *rsa_ctx)
{
	...
	ret = mbedtls_rsa_export_raw(rsa_ctx, n->modulus, n->modulus_len, NULL, 0, NULL, 0, NULL, 0,
				     NULL, 0);
	if (ret != 0) {
		free(n->modulus);
		n->modulus = NULL;
...
static int decrypt_data_from_ta(struct trusted_input *user_input)
{
	...
	ret = mbedtls_rsa_gen_key(&rsa_ctx, mbedtls_ctr_drbg_random, &ctr_drbg_ctx, RSA_KEY_BITS,
				  RSA_EXPONENT);
	if (ret != 0)
		goto err_2;
	...
	encrypted_data.rsa_encrypted_aes_key = malloc(encrypted_data.rsa_encrypted_aes_key_len);
	if (encrypted_data.rsa_encrypted_aes_key == NULL)
		goto err_3;

	encrypted_data.aes_encrypted_user_data_len = 1024;
	encrypted_data.aes_encrypted_user_data = malloc(encrypted_data.aes_encrypted_user_data_len);
	...
	ret = deserialize_data(user_input, encrypted_data.aes_encrypted_user_data,
